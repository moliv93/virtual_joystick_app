...
class CanvasConeButton extends CanvasInteractor {
  ...
  CanvasConeButton({String text = 'E',
                double bigRadius = 40,
                double smallRadius = 35,
                double centerX = 500,
                double centerY = 500,
...
class JoystickPainter extends CustomPainter {
  ...
  void loadTrueCoordinates() async {
    ...
    buttons['W']?.center = Offset(prefs.getDouble('PizzaButtonsX') ?? 0, prefs.getDouble('PizzaButtonsY') ?? 0);
    buttons['N']?.center = Offset(prefs.getDouble('PizzaButtonsX') ?? 0, prefs.getDouble('PizzaButtonsY') ?? 0);
    buttons['S']?.center = Offset(prefs.getDouble('PizzaButtonsX') ?? 0, prefs.getDouble('PizzaButtonsY') ?? 0);
    buttons['E']?.center = Offset(prefs.getDouble('PizzaButtonsX') ?? 0, prefs.getDouble('PizzaButtonsY') ?? 0);
  ...
  JoystickPainter({required this.notifier}) : super(repaint: notifier) {
   handler = CanvasHandler(
    ...
    buttons['S'] = CanvasConeButton(
      text: 'S',
      centerX: 700,
      centerY: 200,
      smallRadius: 100,
      bigRadius: 110,
      normalColor: Colors.pink.shade800,
      pressedColor: Colors.pink.shade400,
      startAngle: (pi/4),
      sweepAngle: pi/2,
    );

    buttons['E'] = CanvasConeButton(
      text: 'E',
      centerX: 700,
      centerY: 200,
      smallRadius: 100,
    ...
    buttons['W'] = CanvasConeButton(
      text: 'W',
      centerX: 700,
      centerY: 200,
      smallRadius: 100,
      bigRadius: 110,
      normalColor: Colors.purple.shade800,
      pressedColor: Colors.purple.shade400,
      startAngle: (pi/4)*3,
      sweepAngle: pi/2,
    );
  ...
  String generateButtonData() {
    String data = '';
    data = data + (buttons['S']!.isPressed ? '1' : '0');
    data = data + (buttons['E']!.isPressed ? '1' : '0');
    data = data + (buttons['N']!.isPressed ? '1' : '0');
    data = data + (buttons['W']!.isPressed ? '1' : '0');
